...
=======
# routers/reports.py — CEO analytics and reporting endpoints
from typing import List, Optional
from decimal import Decimal
from datetime import date, datetime

>>>>>>> fde584f015b042f8f5b84349e02ea264d0fb0462
...
def get_kpi_summary(
    ...
    """Top-level KPI cards for the CEO overview dashboard."""
    date_filter = ""
    params = {}
    if date_from:
        date_filter += " AND s.sale_date >= :date_from"
        params["date_from"] = date_from
    if date_to:
        date_filter += " AND s.sale_date <= :date_to"
...
=======
    """)
    row = db.execute(sales_sql, params).mappings().one()

    gross = row["gross_revenue"] or Decimal("0")
    profit = row["net_profit"] or Decimal("0")
    margin = float(profit / gross * 100) if gross > 0 else 0.0

    # Debtors with positive balance
    debtor_count = db.execute(text("""
        SELECT COUNT(*) FROM (
            SELECT DISTINCT ON (customer_id) customer_id, balance_after
    ...
    return KPISummary(
        ...
        total_outstanding=row["total_outstanding"] or Decimal("0"),
        margin_percent=round(margin, 2),
        total_debtors=debtor_count or 0,
        expiring_batches=expiring or 0,
...
def get_daily_summary(
    ...
    """Daily sales summary for trend charts — last N days."""
>>>>>>> fde584f015b042f8f5b84349e02ea264d0fb0462
...
def get_debtors(
    ...
    """All customers with a positive (owed) balance. CEO only."""
    rows = db.execute(text("""
        SELECT
            c.id            AS customer_id,
            c.full_name,
...
def get_category_performance(
    ...
    """Revenue and profit breakdown by drug category. Powers the CEO bar chart."""
    date_filter = ""
    params = {}
    if date_from:
        date_filter += " AND s.sale_date >= :date_from"
...
def get_staff_performance(
    ...
    """Per-staff sales breakdown. CEO only."""
    date_filter = ""
    params = {}
    if date_from:
        date_filter += " AND s.sale_date >= :date_from"
    ...
    rows = db.execute(text(f"""
        SELECT
            ...
            COALESCE(SUM(si.profit), 0)          AS total_profit
        FROM users u
        LEFT JOIN sales s ON s.served_by = u.id
            AND 1=1 {date_filter}
        LEFT JOIN (
            SELECT sale_id, SUM(line_profit) AS profit
            FROM sale_items GROUP BY sale_id
        ) si ON si.sale_id = s.id
        WHERE u.is_active = TRUE AND u.role = 'manager'
        GROUP BY u.id
